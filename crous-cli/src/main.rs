...
#[command(
    name = "crous",
    version,
    about = "Inspect, validate, and convert Crous data",
    long_about = "crous is a command-line tool for working with Crous binary files and text notation."
)]
struct Cli {
...
enum Commands {
    ...
    /// Pretty-print a .crous file as Crous text notation.
    Pretty(PrettyArgs),
    ...
    /// Parse Crous text notation and emit .crous binary.
    Encode(EncodeArgs),

    /// Decode .crous binary to Crous text notation.
    Decode(DecodeArgs),

    /// Validate header, checksums, trailer, and decode integrity.
...
#[derive(Args, Debug)]
struct InspectArgs {
    /// Input .crous file path (use '-' for stdin).
    file: PathBuf,
}
...
struct FromJsonArgs {
    ...
    /// Output path (default: input path with .crous extension).
    #[arg(short, long)]
    output: Option<PathBuf>,
    ...
    #[arg(long, value_enum, default_value_t = CompressionArg::None, env = "CROUS_COMPRESSION")]
    compression: CompressionArg,
}

#[derive(Args, Debug)]
struct EncodeArgs {
    /// Input Crous text file path (use '-' for stdin).
    file: PathBuf,
...
enum CliError {
    ...
    #[error(transparent)]
    Core(#[from] crous_core::CrousError),
}
...
fn cmd_inspect(ui: &Ui, args: &InspectArgs) -> CliResult<()> {
    ...
    println!("Flags: 0x{:02x}", report.header.flags);
    println!("  has_index:  {}", report.header.has_index());
    println!("  has_schema: {}", report.header.has_schema());
    println!();
...
fn cmd_pretty(_ui: &Ui, args: &PrettyArgs) -> CliResult<()> {
    ...
    let text = values
        .iter()
        .map(|value| crous_core::text::pretty_print(value, args.indent))
        .collect::<Vec<_>>()
        .join("\n\n");
...
fn cmd_from_json(ui: &Ui, args: &FromJsonArgs) -> CliResult<()> {
    ...
    let crous_value = Value::from(&json_value);
    ...
    let out_path = match &args.output {
        Some(path) => path.clone(),
        None => derive_output_path(&args.file, "crous")?,
    };
...
fn cmd_bench(ui: &Ui, args: &BenchArgs) -> CliResult<()> {
    ...
    for _ in 0..args.warmup {
        ...
        encoder.encode_value(&crous_value)?;
    ...
    println!("Crous size: {} bytes", last_bytes.len());
...
mod tests {
    ...
    fn derive_output_path_works_for_regular_file() {
        ...
        let output = derive_output_path(input, "crous").expect("should derive output path");
        assert_eq!(output, PathBuf::from("data/input.crous"));
    }

    #[test]
    fn derive_output_path_rejects_stdin() {
        let input = Path::new("-");
        let err = derive_output_path(input, "crous").expect_err("stdin should fail");
