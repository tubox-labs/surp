...
The RFC-001 surface is a triangle: every producer normalises into the **Canonical Tree Notation** (CTN), every byte sink frames through **Canonical Binary Framing** (CBF), and every reader queries through **Canonical Query Language** (CQL). Nothing reaches a wire byte without passing through the canonical core in the middle.
...
Inside that core, the encode path is a five-stage pipeline. Each stage has a single responsibility and a single set of invariants — the trailer at the end is what makes any of it auditable.
