...
fn parse_steps(expr: &str) -> Result<Vec<Step>> {
    let mut s = expr.trim();
    if s.is_empty() {
        return Err(CrousError::InvalidData("empty CQL expression".into()));
    }
    if !s.starts_with('.') {
...
fn find_matching_bracket(s: &str) -> Result<usize> {
    ...
    for (idx, ch) in s.char_indices() {
        ...
        match ch {
            ...
            ']' => {
                if depth == 0 {
                    return Err(CrousError::InvalidData("unbalanced bracket in CQL".into()));
                }
                depth -= 1;
                if depth == 0 {
                    return Ok(idx);
    ...
    match step {
        ...
        Step::Index(index) => match node {
            Value::Sequence(seq) => {
                ...
                if idx >= 0 {
                    let idx =
                        usize::try_from(idx).map_err(|_| CrousError::LengthOverflow(idx as u64))?;
