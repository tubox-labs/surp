//! RFC-001 CBF (Crous Binary Format) encoder/decoder.
//!
...
pub const CBF_MAGIC: [u8; 4] = *b"CROU";
...
impl CbfHeader {
    ...
    pub fn decode(data: &[u8]) -> Result<Self> {
        ...
        if cbf_version != 0x01 {
            return Err(CrousError::UnsupportedVersion(cbf_version));
        }
...
pub fn encode_document(doc: &Document, options: EncodeOptions) -> Result<Vec<u8>> {
    ...
    let symbol_list: Vec<String> = if options.with_symtab {
        ...
        if contains_symbol_value(&root) {
            return Err(CrousError::InvalidData(
...
pub fn decode_document(data: &[u8]) -> Result<DecodedDocument> {
    ...
    let symbols = if header.has_symtab() {
        let start = usize::try_from(header.symtab_offset)
            .map_err(|_| CrousError::LengthOverflow(header.symtab_offset as u64))?;
    ...
    let root_offset = usize::try_from(header.root_offset)
        .map_err(|_| CrousError::LengthOverflow(header.root_offset))?;
    if root_offset >= payload.len() {
        return Err(CrousError::UnexpectedEof(root_offset));
    }

    let (root, consumed) = decode_segment(payload, root_offset, &symbols, 0)?;
    if root_offset + consumed > payload.len() {
        return Err(CrousError::UnexpectedEof(root_offset + consumed));
    }
...
fn parse_segment_header(data: &[u8], offset: usize) -> Result<SegmentHeader> {
    if offset + 4 > data.len() {
        return Err(CrousError::UnexpectedEof(offset));
    }
    ...
    if len == 0xFF_FFFF {
        if offset + 12 > data.len() {
            return Err(CrousError::UnexpectedEof(offset + 4));
        }
        ...
        len = usize::try_from(big_len).map_err(|_| CrousError::LengthOverflow(big_len))?;
        header_len = 12;
    }
    ...
    let (ty, cfg) = match value {
        ...
        Value::Scalar(Scalar::Sym(sym)) => {
            ...
            let idx = ctx.symbols.get(sym).ok_or_else(|| {
                CrousError::InvalidData(format!("missing symbol table entry '{sym}'"))
            })?;
...
fn encode_sequence(seq: &Sequence, ctx: &EncodeCtx, payload: &mut Vec<u8>) -> Result<()> {
    ...
    for item in &encoded_items {
        let off = u32::try_from(running).map_err(|_| {
            CrousError::InvalidData("sequence segment payload exceeds u32 offsets".into())
        })?;
...
fn encode_map(map: &[(Value, Value)], ctx: &EncodeCtx, payload: &mut Vec<u8>) -> Result<()> {
    ...
    for pair in &encoded_pairs {
        let off = u32::try_from(running).map_err(|_| {
            CrousError::InvalidData("map segment payload exceeds u32 offsets".into())
        })?;
        payload.extend_from_slice(&off.to_le_bytes());
...
fn encode_tensor(tensor: &Tensor, payload: &mut Vec<u8>) -> Result<()> {
    ...
    payload.push(
        u8::try_from(tensor.shape.len())
            .map_err(|_| CrousError::InvalidData("tensor dimensionality exceeds u8".into()))?,
    );
...
fn encode_field_name(name: &str, ctx: &EncodeCtx, payload: &mut Vec<u8>) -> Result<()> {
    if ctx.has_symtab {
        let idx = ctx.symbols.get(name).ok_or_else(|| {
            CrousError::InvalidData(format!("missing symbol table entry for field '{name}'"))
        })?;
...
fn decode_segment(
    ...
    if depth > 64 {
        return Err(CrousError::NestingTooDeep(depth, 64));
    }
    ...
    let payload_end = payload_start
        ...
        .ok_or(CrousError::LengthOverflow(header.payload_len as u64))?;
    ...
    let value = match (header.type_nibble, header.config_nibble) {
        ...
        (TYPE_STRING, _) => {
            let s = std::str::from_utf8(payload)
                .map_err(|_| CrousError::InvalidUtf8(payload_start))?
                .to_string();
            Value::Scalar(Scalar::Str(s))
        }
        ...
        (TYPE_SYMBOL, _) => {
            ...
            let idx = usize::try_from(idx).map_err(|_| CrousError::LengthOverflow(idx))?;
            let sym = symbols
                .get(idx)
                .ok_or(CrousError::InvalidReference(idx, symbols.len()))?;
...
fn decode_product(payload: &[u8], symbols: &[String], depth: usize) -> Result<Value> {
    ...
    pos += consumed;
    let field_count =
        usize::try_from(field_count).map_err(|_| CrousError::LengthOverflow(field_count))?;
...
fn decode_sum(payload: &[u8], symbols: &[String], depth: usize) -> Result<Value> {
    ...
    let variant_payload = match payload_kind {
        ...
        1 => {
            ...
            pos += consumed;
            let count = usize::try_from(count).map_err(|_| CrousError::LengthOverflow(count))?;
    ...
    if pos != payload.len() {
        return Err(CrousError::InvalidData(
            "enum payload trailing bytes".into(),
        ));
    }
...
fn decode_sequence(payload: &[u8], symbols: &[String], depth: usize, cfg: u8) -> Result<Value> {
    ...
    if has_offsets {
        ...
        let table_len = count
            .checked_mul(4)
            .ok_or(CrousError::LengthOverflow(count as u64))?;
        let table_end = table_start
            .checked_add(table_len)
            .ok_or(CrousError::LengthOverflow(table_len as u64))?;
        if table_end > payload.len() {
            return Err(CrousError::UnexpectedEof(table_end));
        }
        ...
        for idx in 0..count {
            ...
            let item_start = table_end
                .checked_add(off)
                .ok_or(CrousError::LengthOverflow(off as u64))?;
            if item_start >= payload.len() {
                return Err(CrousError::UnexpectedEof(item_start));
            }
    ...
    if has_offsets {
        ...
        for idx in 0..count {
            ...
            if pair_start >= payload.len() {
                return Err(CrousError::UnexpectedEof(pair_start));
            }
...
fn decode_reference(payload: &[u8], symbols: &[String], depth: usize) -> Result<Value> {
    if payload.is_empty() {
        return Err(CrousError::UnexpectedEof(0));
    }
    ...
    match kind {
        0 => {
            ...
            let len = usize::try_from(len).map_err(|_| CrousError::LengthOverflow(len))?;
            let start = 1 + consumed;
            let end = start
                .checked_add(len)
                .ok_or(CrousError::LengthOverflow(len as u64))?;
            if end > payload.len() {
                return Err(CrousError::UnexpectedEof(end));
            }
            let name = std::str::from_utf8(&payload[start..end])
                .map_err(|_| CrousError::InvalidUtf8(start))?
                .to_string();
        ...
        _ => Err(CrousError::InvalidData(format!(
...
fn decode_tensor(payload: &[u8]) -> Result<Value> {
    if payload.len() < 4 {
        return Err(CrousError::UnexpectedEof(payload.len()));
    }
    ...
    let data = match payload[0] {
        0x01 | 0x02 => {
            let bytes_needed = element_count
                .checked_mul(8)
                .ok_or(CrousError::LengthOverflow(element_count as u64))?;
            if pos + bytes_needed > payload.len() {
                return Err(CrousError::UnexpectedEof(pos + bytes_needed));
            }
...
fn decode_stream(payload: &[u8], symbols: &[String], depth: usize) -> Result<Value> {
    ...
    pos += consumed_count;
    let ann_count =
        usize::try_from(ann_count).map_err(|_| CrousError::LengthOverflow(ann_count))?;
...
fn decode_opaque(payload: &[u8], cfg: u8) -> Result<Value> {
    ...
    let tag_len = usize::try_from(tag_len).map_err(|_| CrousError::LengthOverflow(tag_len))?;

    let tag_end = pos
        .checked_add(tag_len)
        .ok_or(CrousError::LengthOverflow(tag_len as u64))?;
    if tag_end > payload.len() {
        return Err(CrousError::UnexpectedEof(tag_end));
    }

    let tag = std::str::from_utf8(&payload[pos..tag_end])
        .map_err(|_| CrousError::InvalidUtf8(pos))?
        .to_string();
    pos = tag_end;
    ...
    let value_len =
        usize::try_from(value_len).map_err(|_| CrousError::LengthOverflow(value_len))?;
    let value_end = pos
        .checked_add(value_len)
        .ok_or(CrousError::LengthOverflow(value_len as u64))?;
    if value_end > payload.len() {
        return Err(CrousError::UnexpectedEof(value_end));
    }
    ...
    if cfg == 0 {
        let value = String::from_utf8(value_bytes)
            .map_err(|e| CrousError::InvalidData(format!("invalid opaque tagged utf8: {e}")))?;
...
mod tests {
    ...
    fn crc_detects_corruption() {
        ...
        assert!(matches!(err, CrousError::ChecksumMismatch { .. }));
    }
