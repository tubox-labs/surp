...
fn parse_inline_expr(input: &str) -> Result<Value> {
    let s = input.trim();
    if s.is_empty() {
        return Err(CrousError::InvalidData("empty inline expression".into()));
    }
    ...
    if let Some(b64) = s.strip_prefix("b64\"") {
        ...
        let bytes = base64::engine::general_purpose::STANDARD
            .decode(payload)
            .map_err(|e| CrousError::InvalidBase64(e.to_string()))?;
    ...
    if let Some(sym) = s.strip_prefix('\'') {
        if sym.is_empty() || !is_identifier(sym) {
            return Err(CrousError::InvalidData(format!("invalid symbol '{sym}'")));
        }
    ...
    Err(CrousError::InvalidData(format!(
...
fn parse_inline_sum(s: &str) -> Result<Value> {
    ...
    let rhs = right.trim();
    if rhs.is_empty() {
        return Err(CrousError::InvalidData("missing enum variant".into()));
    }

    if let Some(open) = rhs.find('(') {
        if !rhs.ends_with(')') {
            return Err(CrousError::InvalidData("unterminated enum payload".into()));
        }
...
fn parse_inline_map(s: &str) -> Result<Value> {
    let open = s
        .find('[')
        .ok_or_else(|| CrousError::InvalidData("invalid inline map".into()))?;
    ...
    let scalar = match suffix {
        "u8" | "u16" | "u32" | "u64" | "u128" | "vu32" | "vu64" => {
            ...
            let value = u64::try_from(value_i128).map_err(|_| {
                CrousError::InvalidData(format!("unsigned literal out of range: {s}"))
            })?;
        ...
        "i8" | "i16" | "i32" | "i64" | "i128" => {
            let value = i64::try_from(value_i128).map_err(|_| {
                CrousError::InvalidData(format!("signed literal out of range: {s}"))
            })?;
            Scalar::I64(value)
        }
        "vi32" | "vi64" | "" => {
...
fn parse_float_text(text: &str) -> Result<f64> {
    ...
    cleaned
        .parse::<f64>()
        .map_err(|_| CrousError::InvalidData(format!("invalid float literal '{text}'")))
}
...
fn parse_integer_text(text: &str) -> Result<i128> {
    ...
    let magnitude = i128::from_str_radix(&digits, radix)
        .map_err(|_| CrousError::InvalidData(format!("invalid integer literal '{text}'")))?;
        ...
        match esc {
            ...
            'u' => {
                ...
                let code = u32::from_str_radix(&hex, 16)
                    .map_err(|_| CrousError::InvalidData("invalid unicode escape".into()))?;
                let ch = char::from_u32(code).ok_or_else(|| {
                    CrousError::InvalidData("invalid unicode scalar value".into())
                })?;
                out.push(ch);
            }
            other => {
...
fn parse_hex_compact(hex: &str) -> Result<Vec<u8>> {
    ...
    while idx < bytes.len() {
        let pair = std::str::from_utf8(&bytes[idx..idx + 2])
            .map_err(|_| CrousError::InvalidData("invalid hex bytes literal".into()))?;
...
fn parse_hex_spaced(hex: &str) -> Result<Vec<u8>> {
    ...
    for token in hex.split_whitespace() {
        ...
        let byte = u8::from_str_radix(token, 16)
            .map_err(|_| CrousError::InvalidData("invalid hex byte in literal".into()))?;
        out.push(byte);
    }
    Ok(out)
}
...
fn parse_annotation_line(text: &str) -> Result<Annotation> {
    ...
    if body.is_empty() {
        return Err(CrousError::InvalidData("annotation name missing".into()));
    }
...
fn parse_shape(header: &str) -> Result<Vec<Option<u64>>> {
    ...
    let Some(close) = header.rfind(']') else {
        return Err(CrousError::InvalidData("unterminated tensor shape".into()));
    };
    if close <= open {
        return Ok(Vec::new());
    }
    ...
    for token in split_top_level(inside, ',') {
        ...
        if token == "_" {
            ...
            let scalar = parse_numeric_literal(token)?
                .ok_or_else(|| CrousError::InvalidData("invalid tensor dimension".into()))?;
...
fn parse_err(line: usize, message: String) -> CrousError {
