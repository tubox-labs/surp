//! Trait for types that can be serialized/deserialized with the Surp format.
//!
//! This is the trait generated by `#[derive(Surp)]`. It provides schema-bound
//! encoding with stable field IDs and schema fingerprints.
//!
//! # Blanket implementations
//!
//! The following types have built-in `Surp` implementations:
//!
//! | Rust type | Surp `Value` | Notes |
//! |-----------|---------------|-------|
//! | `bool` | `Bool` | |
//! | `u8`, `u16`, `u32`, `u64`, `usize` | `UInt` | widened to `u64` |
//! | `u128` | `UInt` | panics if value > `u64::MAX` |
//! | `i8`, `i16`, `i32`, `i64`, `isize` | `Int` | widened to `i64` |
//! | `i128` | `Int` | panics if value outside `i64` range |
//! | `f32`, `f64` | `Float` | `f32` widened to `f64` |
//! | `String`, `Box<str>` | `Str` | |
//! | `Vec<u8>` | `Array` of `UInt` | use [`SurpBytes`] for `Value::Bytes` |
//! | `Vec<T: Surp>` | `Array` | |
//! | `Option<T: Surp>` | `T` or `Null` | |
//! | `Box<T: Surp>` | delegates to `T` | |
//! | [`SurpBytes`] | `Bytes` | newtype for raw binary blobs |
//! | `HashMap<String, T>` | `Object` | insertion-order not guaranteed |
//! | `BTreeMap<String, T>` | `Object` | sorted by key |
//! | `(A,)` through `(A,B,C,D,E,F)` | `Array` | heterogeneous tuples |

use crate::error::Result;
use crate::value::Value;
use std::collections::{BTreeMap, HashMap};

/// Trait for types that can encode/decode to/from the Surp binary format.
///
/// Implementations are typically generated by `#[derive(Surp)]` from
/// the `surp-derive` crate, which assigns stable field IDs and computes
/// schema fingerprints.
///
/// # Example (manual implementation)
/// ```
/// use surp_core::{Surp, Value};
///
/// struct Point { x: f64, y: f64 }
///
/// impl Surp for Point {
///     fn to_surp_value(&self) -> Value {
///         Value::Object(vec![
///             ("x".into(), Value::Float(self.x)),
///             ("y".into(), Value::Float(self.y)),
///         ])
///     }
///
///     fn from_surp_value(value: &Value) -> surp_core::Result<Self> {
///         match value {
///             Value::Object(fields) => {
///                 let mut x = 0.0;
///                 let mut y = 0.0;
///                 for (k, v) in fields {
///                     match k.as_str() {
///                         "x" => x = v.as_float().unwrap_or(0.0),
///                         "y" => y = v.as_float().unwrap_or(0.0),
///                         _ => {} // skip unknown fields for forward compat
///                     }
///                 }
///                 Ok(Point { x, y })
///             }
///             _ => Err(surp_core::SurpError::SchemaMismatch(
///                 "expected object for Point".into()
///             )),
///         }
///     }
///
///     fn schema_fingerprint() -> u64 { 0x1234567890abcdef }
///     fn type_name() -> &'static str { "Point" }
/// }
/// ```
pub trait Surp: Sized {
    /// Convert this value to an owned `Value` for encoding.
    fn to_surp_value(&self) -> Value;

    /// Construct this type from a decoded `Value`.
    fn from_surp_value(value: &Value) -> Result<Self>;

    /// A stable fingerprint (XXH64) of this type's schema.
    /// Used for schema-on-write validation.
    fn schema_fingerprint() -> u64;

    /// The human-readable type name for diagnostics.
    fn type_name() -> &'static str;

    /// Encode this value directly to Surp binary bytes.
    ///
    /// Convenience method that creates an `Encoder`, encodes the value,
    /// and returns the finished byte vector.
    fn to_surp_bytes(&self) -> Result<Vec<u8>> {
        let val = self.to_surp_value();
        let mut enc = crate::encoder::Encoder::new();
        enc.encode_value(&val)?;
        enc.finish()
    }

    /// Decode from Surp binary bytes using default limits.
    fn from_surp_bytes(data: &[u8]) -> Result<Self> {
        let mut dec = crate::decoder::Decoder::new(data);
        let val = dec.decode_next()?.to_owned_value();
        Self::from_surp_value(&val)
    }

    /// Decode from Surp binary bytes with custom limits.
    ///
    /// This enforces all `Limits` during binary decode: nesting depth,
    /// memory, block size, string length, and item count.
    fn from_surp_bytes_with_limits(data: &[u8], limits: crate::limits::Limits) -> Result<Self> {
        let mut dec = crate::decoder::Decoder::with_limits(data, limits);
        let val = dec.decode_next()?.to_owned_value();
        Self::from_surp_value(&val)
    }
}

// ---------------------------------------------------------------------------
// Blanket impls for primitive types
// ---------------------------------------------------------------------------

// --- Bool ---

impl Surp for bool {
    fn to_surp_value(&self) -> Value {
        Value::Bool(*self)
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Bool(b) => Ok(*b),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected bool".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0004
    }
    fn type_name() -> &'static str {
        "bool"
    }
}

// --- Unsigned integers ---

/// Helper macro: implement Surp for unsigned integer types that fit in u64.
macro_rules! impl_surp_uint {
    ($($ty:ty => $fp:expr, $name:expr);+ $(;)?) => { $(
        impl Surp for $ty {
            fn to_surp_value(&self) -> Value {
                Value::UInt(*self as u64)
            }

            fn from_surp_value(value: &Value) -> Result<Self> {
                match value {
                    Value::UInt(n) => {
                        <$ty>::try_from(*n).map_err(|_| crate::error::SurpError::SchemaMismatch(
                            format!("uint {} out of range for {}", n, $name)
                        ))
                    }
                    _ => Err(crate::error::SurpError::SchemaMismatch(
                        format!("expected uint for {}", $name)
                    )),
                }
            }

            fn schema_fingerprint() -> u64 { $fp }
            fn type_name() -> &'static str { $name }
        }
    )+ };
}

impl_surp_uint! {
    u8   => 0x0002_0001, "u8";
    u16  => 0x0002_0002, "u16";
    u32  => 0x0002_0003, "u32";
    u64  => 0x0002,      "u64";
}

// usize: same wire representation as u64
impl Surp for usize {
    fn to_surp_value(&self) -> Value {
        Value::UInt(*self as u64)
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::UInt(n) => usize::try_from(*n).map_err(|_| {
                crate::error::SurpError::SchemaMismatch(format!("uint {n} out of range for usize"))
            }),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected uint for usize".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0002_0005
    }
    fn type_name() -> &'static str {
        "usize"
    }
}

// u128: encode as UInt, but panics on encode if > u64::MAX
impl Surp for u128 {
    fn to_surp_value(&self) -> Value {
        Value::UInt(u64::try_from(*self).expect("u128 value exceeds u64::MAX for Surp encoding"))
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::UInt(n) => Ok(u128::from(*n)),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected uint for u128".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0002_0006
    }
    fn type_name() -> &'static str {
        "u128"
    }
}

// --- Signed integers ---

/// Helper macro: implement Surp for signed integer types that fit in i64.
macro_rules! impl_surp_int {
    ($($ty:ty => $fp:expr, $name:expr);+ $(;)?) => { $(
        impl Surp for $ty {
            fn to_surp_value(&self) -> Value {
                Value::Int(i64::from(*self))
            }

            fn from_surp_value(value: &Value) -> Result<Self> {
                match value {
                    Value::Int(n) => {
                        <$ty>::try_from(*n).map_err(|_| crate::error::SurpError::SchemaMismatch(
                            format!("int {} out of range for {}", n, $name)
                        ))
                    }
                    // Also accept UInt for small non-negative values (cross-compat).
                    Value::UInt(n) => {
                        let as_i64 = i64::try_from(*n).map_err(|_| {
                            crate::error::SurpError::SchemaMismatch(
                                format!("uint {} out of range for {}", n, $name)
                            )
                        })?;
                        <$ty>::try_from(as_i64).map_err(|_| {
                            crate::error::SurpError::SchemaMismatch(
                                format!("uint {} out of range for {}", n, $name)
                            )
                        })
                    }
                    _ => Err(crate::error::SurpError::SchemaMismatch(
                        format!("expected int for {}", $name)
                    )),
                }
            }

            fn schema_fingerprint() -> u64 { $fp }
            fn type_name() -> &'static str { $name }
        }
    )+ };
}

impl_surp_int! {
    i8   => 0x0003_0001, "i8";
    i16  => 0x0003_0002, "i16";
    i32  => 0x0003_0003, "i32";
    i64  => 0x0003,      "i64";
}

// isize: same wire representation as i64
impl Surp for isize {
    fn to_surp_value(&self) -> Value {
        Value::Int(*self as i64)
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Int(n) => isize::try_from(*n).map_err(|_| {
                crate::error::SurpError::SchemaMismatch(format!("int {n} out of range for isize"))
            }),
            Value::UInt(n) => {
                let as_i64 = i64::try_from(*n).map_err(|_| {
                    crate::error::SurpError::SchemaMismatch(format!(
                        "uint {n} out of range for isize"
                    ))
                })?;
                isize::try_from(as_i64).map_err(|_| {
                    crate::error::SurpError::SchemaMismatch(format!(
                        "int {as_i64} out of range for isize"
                    ))
                })
            }
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected int for isize".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0003_0005
    }
    fn type_name() -> &'static str {
        "isize"
    }
}

// i128: encode as Int, but panics on encode if outside i64 range
impl Surp for i128 {
    fn to_surp_value(&self) -> Value {
        Value::Int(i64::try_from(*self).expect("i128 value exceeds i64 range for Surp encoding"))
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Int(n) => Ok(i128::from(*n)),
            Value::UInt(n) => Ok(i128::from(*n)),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected int for i128".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0003_0006
    }
    fn type_name() -> &'static str {
        "i128"
    }
}

// --- Floating point ---

impl Surp for f64 {
    fn to_surp_value(&self) -> Value {
        Value::Float(*self)
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Float(f) => Ok(*f),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected float".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0005
    }
    fn type_name() -> &'static str {
        "f64"
    }
}

impl Surp for f32 {
    fn to_surp_value(&self) -> Value {
        Value::Float(f64::from(*self))
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Float(f) => Ok(*f as f32),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected float for f32".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0005_0001
    }
    fn type_name() -> &'static str {
        "f32"
    }
}

// --- String types ---

impl Surp for String {
    fn to_surp_value(&self) -> Value {
        Value::Str(self.clone())
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Str(s) => Ok(s.clone()),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected string".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0001
    }
    fn type_name() -> &'static str {
        "String"
    }
}

impl Surp for Box<str> {
    fn to_surp_value(&self) -> Value {
        Value::Str(self.to_string())
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Str(s) => Ok(s.clone().into_boxed_str()),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected string for Box<str>".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0001_0001
    }
    fn type_name() -> &'static str {
        "Box<str>"
    }
}

// --- Raw bytes newtype ---

/// Newtype wrapper for `Vec<u8>` that serializes as `Value::Bytes` (binary blob).
///
/// Use this instead of `Vec<u8>` when you want the raw-bytes wire encoding.
/// Plain `Vec<u8>` encodes as an array of unsigned integers via the generic
/// `Vec<T: Surp>` impl.
///
/// # Example
/// ```
/// use surp_core::{Surp, SurpBytes, Value};
///
/// let blob = SurpBytes(vec![0xDE, 0xAD, 0xBE, 0xEF]);
/// assert!(matches!(blob.to_surp_value(), Value::Bytes(_)));
///
/// let arr: Vec<u8> = vec![1, 2, 3];
/// // Vec<u8> uses the generic Vec<T> impl → Array of UInts
/// assert!(matches!(arr.to_surp_value(), Value::Array(_)));
/// ```
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct SurpBytes(pub Vec<u8>);

impl SurpBytes {
    /// Create a new `SurpBytes` from a byte vector.
    pub fn new(bytes: Vec<u8>) -> Self {
        Self(bytes)
    }

    /// Consume and return the inner `Vec<u8>`.
    pub fn into_inner(self) -> Vec<u8> {
        self.0
    }

    /// Borrow the inner bytes as a slice.
    pub fn as_bytes(&self) -> &[u8] {
        &self.0
    }
}

impl From<Vec<u8>> for SurpBytes {
    fn from(v: Vec<u8>) -> Self {
        Self(v)
    }
}

impl From<SurpBytes> for Vec<u8> {
    fn from(b: SurpBytes) -> Self {
        b.0
    }
}

impl AsRef<[u8]> for SurpBytes {
    fn as_ref(&self) -> &[u8] {
        &self.0
    }
}

impl Surp for SurpBytes {
    fn to_surp_value(&self) -> Value {
        Value::Bytes(self.0.clone())
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Bytes(b) => Ok(SurpBytes(b.clone())),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected bytes".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0006
    }
    fn type_name() -> &'static str {
        "SurpBytes"
    }
}

// --- Generic containers ---

impl<T: Surp> Surp for Vec<T> {
    fn to_surp_value(&self) -> Value {
        Value::Array(self.iter().map(|item| item.to_surp_value()).collect())
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Array(items) => items.iter().map(T::from_surp_value).collect(),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected array".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        T::schema_fingerprint()
            .wrapping_mul(31)
            .wrapping_add(0x0010)
    }

    fn type_name() -> &'static str {
        "Vec"
    }
}

impl<T: Surp> Surp for Option<T> {
    fn to_surp_value(&self) -> Value {
        match self {
            Some(v) => v.to_surp_value(),
            None => Value::Null,
        }
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Null => Ok(None),
            other => Ok(Some(T::from_surp_value(other)?)),
        }
    }

    fn schema_fingerprint() -> u64 {
        T::schema_fingerprint()
            .wrapping_mul(37)
            .wrapping_add(0x0020)
    }

    fn type_name() -> &'static str {
        "Option"
    }
}

impl<T: Surp> Surp for Box<T> {
    fn to_surp_value(&self) -> Value {
        (**self).to_surp_value()
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        T::from_surp_value(value).map(Box::new)
    }

    fn schema_fingerprint() -> u64 {
        T::schema_fingerprint()
    }

    fn type_name() -> &'static str {
        T::type_name()
    }
}

// --- Map types → Object ---

impl<T: Surp> Surp for HashMap<String, T> {
    fn to_surp_value(&self) -> Value {
        Value::Object(
            self.iter()
                .map(|(k, v)| (k.clone(), v.to_surp_value()))
                .collect(),
        )
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Object(entries) => {
                let mut map = HashMap::with_capacity(entries.len());
                for (k, v) in entries {
                    map.insert(k.clone(), T::from_surp_value(v)?);
                }
                Ok(map)
            }
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected object for HashMap".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        T::schema_fingerprint()
            .wrapping_mul(41)
            .wrapping_add(0x0030)
    }

    fn type_name() -> &'static str {
        "HashMap"
    }
}

impl<T: Surp> Surp for BTreeMap<String, T> {
    fn to_surp_value(&self) -> Value {
        Value::Object(
            self.iter()
                .map(|(k, v)| (k.clone(), v.to_surp_value()))
                .collect(),
        )
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Object(entries) => {
                let mut map = BTreeMap::new();
                for (k, v) in entries {
                    map.insert(k.clone(), T::from_surp_value(v)?);
                }
                Ok(map)
            }
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected object for BTreeMap".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        T::schema_fingerprint()
            .wrapping_mul(43)
            .wrapping_add(0x0031)
    }

    fn type_name() -> &'static str {
        "BTreeMap"
    }
}

// --- Tuple impls (heterogeneous, encoded as Array) ---

macro_rules! impl_surp_tuple {
    ($fp:expr, $($idx:tt : $T:ident),+) => {
        impl<$($T: Surp),+> Surp for ($($T,)+) {
            fn to_surp_value(&self) -> Value {
                Value::Array(vec![
                    $(self.$idx.to_surp_value()),+
                ])
            }

            fn from_surp_value(value: &Value) -> Result<Self> {
                match value {
                    Value::Array(items) => {
                        let mut iter = items.iter();
                        Ok(($(
                            $T::from_surp_value(
                                iter.next().ok_or_else(|| {
                                    crate::error::SurpError::SchemaMismatch(
                                        "tuple: not enough array elements".into()
                                    )
                                })?
                            )?,
                        )+))
                    }
                    _ => Err(crate::error::SurpError::SchemaMismatch(
                        "expected array for tuple".into(),
                    )),
                }
            }

            fn schema_fingerprint() -> u64 { $fp }
            fn type_name() -> &'static str { "tuple" }
        }
    };
}

impl_surp_tuple!(0x0040_0001, 0: A);
impl_surp_tuple!(0x0040_0002, 0: A, 1: B);
impl_surp_tuple!(0x0040_0003, 0: A, 1: B, 2: C);
impl_surp_tuple!(0x0040_0004, 0: A, 1: B, 2: C, 3: D);
impl_surp_tuple!(0x0040_0005, 0: A, 1: B, 2: C, 3: D, 4: E);
impl_surp_tuple!(0x0040_0006, 0: A, 1: B, 2: C, 3: D, 4: E, 5: F);

// --- Unit type ---

impl Surp for () {
    fn to_surp_value(&self) -> Value {
        Value::Null
    }

    fn from_surp_value(value: &Value) -> Result<Self> {
        match value {
            Value::Null => Ok(()),
            _ => Err(crate::error::SurpError::SchemaMismatch(
                "expected null for ()".into(),
            )),
        }
    }

    fn schema_fingerprint() -> u64 {
        0x0000
    }
    fn type_name() -> &'static str {
        "()"
    }
}

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn u8_roundtrip() {
        let v: u8 = 200;
        let val = v.to_surp_value();
        assert_eq!(val, Value::UInt(200));
        assert_eq!(u8::from_surp_value(&val).unwrap(), 200);
    }

    #[test]
    fn u8_overflow() {
        let val = Value::UInt(256);
        assert!(u8::from_surp_value(&val).is_err());
    }

    #[test]
    fn u16_roundtrip() {
        let v: u16 = 60000;
        let val = v.to_surp_value();
        assert_eq!(u16::from_surp_value(&val).unwrap(), 60000);
    }

    #[test]
    fn u32_roundtrip() {
        let v: u32 = u32::MAX;
        let val = v.to_surp_value();
        assert_eq!(u32::from_surp_value(&val).unwrap(), u32::MAX);
    }

    #[test]
    fn i8_roundtrip() {
        let v: i8 = -42;
        let val = v.to_surp_value();
        assert_eq!(val, Value::Int(-42));
        assert_eq!(i8::from_surp_value(&val).unwrap(), -42);
    }

    #[test]
    fn i8_overflow() {
        let val = Value::Int(200);
        assert!(i8::from_surp_value(&val).is_err());
    }

    #[test]
    fn i16_roundtrip() {
        let v: i16 = -30000;
        let val = v.to_surp_value();
        assert_eq!(i16::from_surp_value(&val).unwrap(), -30000);
    }

    #[test]
    fn i32_roundtrip() {
        let v: i32 = i32::MIN;
        let val = v.to_surp_value();
        assert_eq!(i32::from_surp_value(&val).unwrap(), i32::MIN);
    }

    #[test]
    fn i32_from_uint_compat() {
        // Signed types should accept UInt values when they fit.
        let val = Value::UInt(42);
        assert_eq!(i32::from_surp_value(&val).unwrap(), 42);
    }

    #[test]
    fn f32_roundtrip() {
        let v: f32 = 2.78;
        let val = v.to_surp_value();
        assert!(matches!(val, Value::Float(_)));
        let back = f32::from_surp_value(&val).unwrap();
        assert!((back - 2.78).abs() < 1e-5);
    }

    #[test]
    fn vec_u8_is_array() {
        let v: Vec<u8> = vec![1, 2, 3];
        let val = v.to_surp_value();
        // Vec<u8> uses the generic Vec<T> impl → Array of UInts.
        assert!(matches!(val, Value::Array(_)));
        let back = Vec::<u8>::from_surp_value(&val).unwrap();
        assert_eq!(back, vec![1, 2, 3]);
    }

    #[test]
    fn surp_bytes_is_bytes() {
        let blob = SurpBytes(vec![0xDE, 0xAD, 0xBE, 0xEF]);
        let val = blob.to_surp_value();
        assert!(matches!(val, Value::Bytes(_)));
        let back = SurpBytes::from_surp_value(&val).unwrap();
        assert_eq!(back.0, vec![0xDE, 0xAD, 0xBE, 0xEF]);
    }

    #[test]
    fn box_str_roundtrip() {
        let s: Box<str> = "hello".into();
        let val = s.to_surp_value();
        assert_eq!(val, Value::Str("hello".into()));
        let back = Box::<str>::from_surp_value(&val).unwrap();
        assert_eq!(&*back, "hello");
    }

    #[test]
    fn box_t_roundtrip() {
        let v: Box<u32> = Box::new(42);
        let val = v.to_surp_value();
        assert_eq!(val, Value::UInt(42));
        let back = Box::<u32>::from_surp_value(&val).unwrap();
        assert_eq!(*back, 42);
    }

    #[test]
    fn hashmap_roundtrip() {
        let mut map = HashMap::new();
        map.insert("x".to_string(), 10u64);
        map.insert("y".to_string(), 20u64);

        let val = map.to_surp_value();
        assert!(matches!(val, Value::Object(_)));

        let back = HashMap::<String, u64>::from_surp_value(&val).unwrap();
        assert_eq!(back.get("x"), Some(&10));
        assert_eq!(back.get("y"), Some(&20));
    }

    #[test]
    fn btreemap_roundtrip() {
        let mut map = BTreeMap::new();
        map.insert("a".to_string(), Value::Bool(true));
        map.insert("b".to_string(), Value::UInt(7));

        // BTreeMap<String, Value> doesn't work since Value doesn't impl Surp,
        // but BTreeMap<String, bool> does.
        let mut m2 = BTreeMap::new();
        m2.insert("flag".to_string(), true);
        m2.insert("enabled".to_string(), false);
        let val = m2.to_surp_value();
        let back = BTreeMap::<String, bool>::from_surp_value(&val).unwrap();
        assert_eq!(back.get("flag"), Some(&true));
        assert_eq!(back.get("enabled"), Some(&false));
    }

    #[test]
    fn tuple2_roundtrip() {
        let t = (42u32, "hello".to_string());
        let val = t.to_surp_value();
        assert!(matches!(val, Value::Array(_)));
        let back = <(u32, String)>::from_surp_value(&val).unwrap();
        assert_eq!(back, (42, "hello".to_string()));
    }

    #[test]
    fn tuple3_roundtrip() {
        let t = (true, -5i16, 2.78f64);
        let val = t.to_surp_value();
        let back = <(bool, i16, f64)>::from_surp_value(&val).unwrap();
        assert!(back.0);
        assert_eq!(back.1, -5);
        assert!((back.2 - 2.78).abs() < 1e-10);
    }

    #[test]
    fn unit_roundtrip() {
        let val = ().to_surp_value();
        assert_eq!(val, Value::Null);
        assert_eq!(<()>::from_surp_value(&val).unwrap(), ());
    }

    #[test]
    fn usize_roundtrip() {
        let v: usize = 999;
        let val = v.to_surp_value();
        assert_eq!(usize::from_surp_value(&val).unwrap(), 999);
    }

    #[test]
    fn isize_roundtrip() {
        let v: isize = -999;
        let val = v.to_surp_value();
        assert_eq!(isize::from_surp_value(&val).unwrap(), -999);
    }

    #[test]
    fn u128_small_roundtrip() {
        let v: u128 = 123456;
        let val = v.to_surp_value();
        assert_eq!(u128::from_surp_value(&val).unwrap(), 123456);
    }

    #[test]
    fn i128_small_roundtrip() {
        let v: i128 = -123456;
        let val = v.to_surp_value();
        assert_eq!(i128::from_surp_value(&val).unwrap(), -123456);
    }

    #[test]
    fn nested_option_vec() {
        let v: Option<Vec<u8>> = Some(vec![10, 20, 30]);
        let val = v.to_surp_value();
        let back = Option::<Vec<u8>>::from_surp_value(&val).unwrap();
        assert_eq!(back, Some(vec![10, 20, 30]));
    }

    #[test]
    fn option_none() {
        let v: Option<u8> = None;
        let val = v.to_surp_value();
        assert_eq!(val, Value::Null);
        let back = Option::<u8>::from_surp_value(&val).unwrap();
        assert_eq!(back, None);
    }

    #[test]
    fn full_binary_roundtrip_u8() {
        // Verify u8 survives encode → binary → decode.
        let v: u8 = 42;
        let bytes = v.to_surp_bytes().unwrap();
        let back = u8::from_surp_bytes(&bytes).unwrap();
        assert_eq!(back, 42);
    }

    #[test]
    fn full_binary_roundtrip_surp_bytes() {
        let blob = SurpBytes(vec![1, 2, 3, 4]);
        let bytes = blob.to_surp_bytes().unwrap();
        let back = SurpBytes::from_surp_bytes(&bytes).unwrap();
        assert_eq!(back.0, vec![1, 2, 3, 4]);
    }

    #[test]
    fn full_binary_roundtrip_tuple() {
        let t = (100u16, "world".to_string(), false);
        let bytes = t.to_surp_bytes().unwrap();
        let back = <(u16, String, bool)>::from_surp_bytes(&bytes).unwrap();
        assert_eq!(back, (100, "world".to_string(), false));
    }

    #[test]
    fn full_binary_roundtrip_hashmap() {
        let mut m = HashMap::new();
        m.insert("key".to_string(), 99u32);
        let bytes = m.to_surp_bytes().unwrap();
        let back = HashMap::<String, u32>::from_surp_bytes(&bytes).unwrap();
        assert_eq!(back.get("key"), Some(&99));
    }
}
